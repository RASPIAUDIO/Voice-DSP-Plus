...
STILL_SCENES = (
    StillScene(
        ...
        "pcb",
        "VOICE DSP+",
        "THE AUDIO FRONT END FOR AI VOICE",
        ("4 microphones", "Playback", "Real-time voice DSP"),
        theme="light",
        rotate_180=True,
        logo=True,
    ),
    StillScene(
        ...
        10.0,
        "usb",
        "USB. PLUG IN. START TALKING.",
        "A standard two-way sound card for fast integration.",
        ("48 kHz", "2 processed mic channels", "Stereo playback", "No product-specific driver"),
    ),
    StillScene(
        "03-io",
        9.0,
        ...
        "Capture a voice while the product answers aloud.",
        ("Onboard speaker", "11 W external speaker output", "Stereo headphone jack"),
        theme="light",
    ),
    StillScene(
        "04-dsp",
    ...
    StillScene(
        ...
        ("I2S audio", "I2C host control", "Tunable DSP", "Standalone voice products"),
        card=True,
    ),
    StillScene(
        "07-applications",
        ...
        ("AI assistants", "Video calls", "Kiosks", "Robotics", "Meeting rooms"),
        theme="light",
        card=True,
    ),
)
...
DEMO_SCENES = (
    ("08-aec", 8.0, "aec_demo", 2.5),
    ("09-noise", 8.0, "noise_demo", 4.0),
    ("10-wake", 8.0, "wake_demo", 8.0),
)
...
def render_demo_scene(key: str, duration: float, source: Path, seek: float, output: Path) -> None:
    filters = (
        ...
        "text='REAL TEST EXCERPT  /  FULL DEMO BELOW':x=54:y=13:fontsize=24:fontcolor=0xF6C879,"
...
def render_close(asset_paths: dict[str, Path], output: Path, regular_font: Path, bold_font: Path) -> None:
    ...
    filters = ";".join(
        (
            ...
            f"drawtext=fontfile='{bold}':text='raspiaudio.com':x=180:y=665:fontsize=44:fontcolor=0xD88428,"
